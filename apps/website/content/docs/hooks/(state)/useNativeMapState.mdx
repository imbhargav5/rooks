...
description: "Manages a native Map object in React state with reactive update helpers."
entrypoint: rooks
status: stable
aliases: []
related: [useMapState, useSetState]
---
...
export default function Scores() {
  const [scores, { set }] = useNativeMapState(
    new Map<string, number>([["Ada", 1]])
  );

  return (
    <section>
      <p>Ada: {scores.get("Ada") ?? 0}</p>
      <button
        type="button"
        onClick={() => set("Ada", (scores.get("Ada") ?? 0) + 1)}
      >
        Add point
      </button>
    </section>
...
Most mutation helpers clone the current `Map` before changing it and then commit the clone. `set` currently calls `Map#set` on the current internal map before cloning, so consumers must treat every returned `ReadonlyMap` as read-only and must not rely on an older snapshot remaining unchanged after `set`. Query helpers (`get`, `has`, `hasSome`, `hasEvery`, and `size`) are recreated when the map changes. The hook creates no effects or external resources.

## Compatibility and accessibility

The hook uses only React state and the built-in `Map` type, so it is SSR-safe and needs no permission. Convert entries to semantic list, table, or form controls when rendering them; `Map` state itself has no accessibility semantics.

## Related

- [useMapState](/docs/hooks/useMapState)
- [useSetState](/docs/hooks/useSetState)
