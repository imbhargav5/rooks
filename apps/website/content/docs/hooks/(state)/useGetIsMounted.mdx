...
description: "Returns a function that reports whether the component is currently mounted."
entrypoint: rooks
status: stable
aliases: []
related: [useSafeSetState]
---
...
The returned function has a stable identity and reads a ref. It returns `false` during render and server rendering, becomes `true` after the hook's effect commits, and returns `false` after effect cleanup. It does not cancel async work; use `AbortController` or an API-specific cancellation mechanism when the operation itself should stop.

## Compatibility and accessibility

The hook is SSR-safe and has no browser or permission requirement. It does not render UI, so accessibility depends on how the guarded async result is presented.

## Related

- [useSafeSetState](/docs/hooks/useSafeSetState)
