  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ padding: "10px", borderRadius: "5px", backgroundColor: 
        ...
        {isGranted && "✅ Permission granted - Location access is available"}
        {isDenied && "❌ Permission denied - Location access is blocked"}
        {isPrompt && "⚠️ Permission prompt - User hasn't decided yet"}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const {
    ...
    onGranted: () => addMessage("Permission granted! 🎉"),
    onDenied: () => addMessage("Permission denied 😞"),
