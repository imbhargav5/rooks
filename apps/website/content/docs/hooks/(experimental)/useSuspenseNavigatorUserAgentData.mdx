...
description: "Reads User-Agent Client Hints with Suspense support."
...
function DeviceInfo() {
  ...
  return (
    <div>
      ...
      <p>
        Form Factors: {userAgentData.formFactors?.join(", ") || "Not available"}
      </p>
    </div>
  );
}
...
The hook sorts the supplied hint array to form a module-level cache key, calls `getHighEntropyValues` once per unique set, and suspends until that promise settles. Resolved results remain cached; the hook creates no event listener or cleanup resource. Because sorting is in place, pass a disposable or already-sorted array if caller-owned order matters.

## Compatibility and accessibility

The hook throws when `navigator.userAgentData` is absent, including during SSR, and rethrows request failures to an error boundary. Availability is primarily Chromium-based and high-entropy values may be reduced by browser privacy policy. Use feature detection for behavior; user-agent data is not a reliable authorization, identity, or accessibility signal.
