...
description: "Reads and writes sessionStorage state with Suspense support."
...
function Counter() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setItem(count - 1)}>Decrement</button>
...
function ContactForm() {
  const [formData, { setItem, getItem, deleteItem }] =
  ...
  return (
    <div style={{ maxWidth: "400px", padding: "20px" }}>
      ...
      <div style={{ marginTop: "20px", display: "flex", gap: "10px" }}>
        <button onClick={getCurrentData}>Log Current Data</button>
...
function MultiStepForm() {
  ...
  return (
    <div style={{ maxWidth: "600px", padding: "20px" }}>
      ...
      <p style={{ fontSize: "12px", color: "#666", marginBottom: "20px" }}>
        Progress is saved in sessionStorage - refresh the page to see
        persistence
      </p>
      ...
      >
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          {formState.step < 3 ? (
            <button onClick={() => updateStep(formState.step + 1)}>Next</button>
          ) : (
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        ...
        <div style={{ marginTop: "10px" }}>Language: {settings.language}</div>
      </div>
...
The first use of a key creates a module-level initialization promise and suspends until `initializer` returns. State changes are JSON-stringified to sessionStorage from an effect. Same-document instances synchronize through a custom event; a `storage` listener is also installed and removed on cleanup, although sessionStorage is normally isolated per top-level tab. Deletion removes the key and resets with `initializer(null)`.

## Compatibility and accessibility

When sessionStorage is unavailable or blocked, the hook logs storage errors and continues with in-memory state. Initializer failures reach the nearest error boundary. SSR initializes from `null`; use deterministic fallback markup. Session data is synchronous, quota-limited, cleared with the tab session, visible to same-origin scripts, and unsuitable for secrets.
