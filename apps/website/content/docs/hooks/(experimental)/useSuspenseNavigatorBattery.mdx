...
description: "Reads battery status from the Battery API with Suspense support."
...
The first call caches `navigator.getBattery()` and suspends until it settles. After resolution, the hook listens for charging, charging-time, discharging-time, and level changes and removes all four listeners on unmount. Every instance uses the same module-level battery promise, while each mounted instance maintains its own current snapshot.

## Compatibility and accessibility

The hook throws when `navigator.getBattery` is absent, including during SSR, and rethrows a rejected API promise to the nearest error boundary. The Battery Status API has limited browser availability and may be restricted for privacy; never gate essential functionality on it. Announce low-battery advice in text rather than relying only on color or an icon.
