...
description: "Reads and writes localStorage state with Suspense support."
...
function UserProfile() {
  ...
  return (
    <div>
      ...
      <button onClick={deleteItem}>Clear Profile</button>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ gap: "10px", display: "flex" }}>
        <button onClick={() => setItem(count + 1)}>+1</button>
        <button onClick={() => setItem(count + 10)}>+10</button>
        <button onClick={deleteItem}>Reset</button>
      </div>
...
The first use of a key creates a module-level initialization promise and suspends until `initializer` returns. State changes are JSON-stringified to localStorage from an effect. Same-document instances synchronize through a custom event; other tabs synchronize through the browser `storage` event. Both listeners are removed on cleanup. Deletion removes the key and resets state with `initializer(null)`.

## Compatibility and accessibility

When localStorage is unavailable or a read/write/delete is blocked, the hook logs the storage error and continues with in-memory state. An initializer error is thrown to the nearest error boundary. SSR initializes from `null`, so choose a deterministic fallback and expect the browser's stored value to be client-only. Storage is synchronous, quota-limited, visible to same-origin scripts, and unsuitable for secrets.
