...
description: "Reads and writes IndexedDB state with Suspense support."
...
type ItemData = { items: string[] };

function CustomDBExample() {
  const [data, { setItem }] = useSuspenseIndexedDBState<ItemData>(
    "my-data",
    (currentValue) =>
      currentValue &&
      typeof currentValue === "object" &&
      "items" in currentValue
        ? (currentValue as ItemData)
        : { items: [] },
    {
      dbName: "my-app-db",
      storeName: "app-data",
...
function SyncDemo() {
  const [message, { setItem, deleteItem }] = useSuspenseIndexedDBState<string>(
    "sync-message",
    (currentValue) =>
      typeof currentValue === "string" ? currentValue : "Hello World"
  );

  const [lastUpdated, { setItem: setLastUpdated }] =
    useSuspenseIndexedDBState<Date>("last-updated", (currentValue) =>
  ...
  return (
    <div>
      ...
      <button onClick={clearAll}>Clear Message</button>

      <p>
        <em>
          Try changing the message in one tab and watch it update in others!
        </em>
      </p>
    </div>
  );
...
| config      | `{ dbName?: string; storeName?: string; version?: number }` | Database configuration                                            | No       |
...
Initialization is cached by database, store, key, and version and suspends until the database read and `initializer` finish. Each read/write/delete opens a database connection and closes it when the transaction completes. `setItem` and `deleteItem` update local state only after the transaction succeeds, then broadcast to other browsing contexts; the shared BroadcastChannel listener is closed on cleanup.

## Compatibility and accessibility

When IndexedDB is missing or the initial read fails, the implementation logs the failure and calls `initializer(null)` rather than surfacing a storage error. Initializer failures suspend into the nearest error boundary; write and delete failures reject their returned promises. Cross-tab updates require BroadcastChannel support. Treat stored values as untrusted after deployment migrations and do not store secrets merely because the API is origin-scoped.
