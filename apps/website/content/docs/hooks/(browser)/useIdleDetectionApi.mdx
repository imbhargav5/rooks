...
description: "Detects when the user is idle using the IdleDetection API."
entrypoint: rooks
status: stable
aliases: []
related: []
...
export default function App() {
  const { isIdle, userState, screenState, isSupported, start, stop } =
    useIdleDetectionApi({
  ...
  return (
    <div>
      ...
      <p>
        User State: <strong>{userState}</strong>
      </p>
      <p>
        Screen State: <strong>{screenState}</strong>
      </p>
      <p>
        Is Idle: <strong>{isIdle ? "Yes" : "No"}</strong>
      </p>
...
export default function App() {
  ...
  const { isIdle, userState, screenState, isPermissionGranted } =
    useIdleDetectionApi({
...
`start` is idempotent until `stop` runs. Native support uses `IdleDetector`; otherwise a fallback marks the user idle after no mouse, keyboard, click, scroll, or touch activity and approximates lock state with document visibility. Both paths clamp the threshold to at least 60 seconds. `stop` resets all public state and fully removes fallback listeners/timers; native change listeners are removed, but the underlying native detector is not aborted by this implementation.

## Compatibility and accessibility

`isSupported` means native Idle Detection support; the fallback can still run when it is false. Native use may require HTTPS, a top-level context, permission, and a user gesture. With `requestPermission: false`, the hook skips the explicit permission request and `start` can still fail through `onError`. Treat idle and visibility as hints, never as proof that a person is absent or a screen is locked, and keep security timeouts independent.
