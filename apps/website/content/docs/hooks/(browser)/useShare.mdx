---
title: useShare
description: "Triggers the native Web Share dialog to share content from the browser."
entrypoint: rooks
status: stable
aliases: []
related: []
...
export default function ShareArticle() {
  const { error, isSharing, isSupported, share } = useShare();
  ...
  return (
    <section>
      <button
        ...
        onClick={() =>
          void share({ title: "Rooks", text: "Typed React hooks" })
        }
      >
        {isSharing ? "Sharing…" : "Share"}
      </button>
...
This hook takes no parameters. `share(data)` accepts `{ title?: string; text?: string; url?: string; files?: File[] }` and returns `Promise<void>`.

## Behavior and lifecycle

`share` clears the old error, sets `isSharing`, and delegates to `navigator.share`. User cancellation (`AbortError`) clears the error and resolves normally. Unsupported use and other failures store an `Error` and reject; `isSharing` resets in all cases. The hook creates no persistent resource and has no cleanup.

## Compatibility and accessibility

SSR reports unsupported. Sharing generally requires HTTPS and a direct user gesture. File support is separate; check `navigator.canShare({ files })` before calling with files. Keep a copy-link or equivalent fallback, label the share control, and do not report cancellation as a failure.
