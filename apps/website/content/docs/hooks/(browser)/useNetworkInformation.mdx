...
description: "Exposes network connectivity and speed information from the Network Information API."
entrypoint: rooks
status: stable
aliases: []
related: []
...
function NetworkStatus() {
  const { effectiveType, downlink, rtt, saveData, type, isSupported } =
...
export default function ConnectionSummary() {
  ...
  return (
    <p>
      Connection: {effectiveType ?? "unknown"}; data saver:{" "}
      {saveData ? "on" : "off"}
    </p>
  );
}
...
The hook reads `navigator.connection` (including `mozConnection` and `webkitConnection` fallbacks) and subscribes to its `change` event. It removes the old listener when the connection object changes or the component unmounts. Missing fields remain `null`; `saveData` defaults to `false`.

## Compatibility and accessibility

SSR and unsupported browsers return null metrics with `isSupported: false`. The API has limited availability and its values are rounded estimates, not a reachability or bandwidth guarantee. Use it to reduce optional work, respect `saveData`, and keep a user-controlled path to full-quality content.
