...
description: "Requests permission and sends desktop notifications via the Notifications API."
entrypoint: rooks
status: stable
aliases: []
related: []
...
This hook takes no parameters. `show(title, options?)` accepts the platform `NotificationOptions` shape shown above.

## Behavior and lifecycle

The initial permission is read from `Notification.permission`. `requestPermission` updates hook state with the returned value. `show` creates and returns a `Notification` only while permission is `"granted"`; otherwise it returns `null`. The hook does not watch browser-settings changes or close notifications on unmount, so callers may close a returned instance when appropriate.

## Compatibility and accessibility

SSR and unsupported browsers report `isSupported: false`; `requestPermission` resolves to `"denied"` and `show` resolves to `null` in that case. API/construction failures are logged rather than returned as hook error state. Request permission only from a clear user action, provide equivalent in-page information, and do not require notifications for essential workflows.
