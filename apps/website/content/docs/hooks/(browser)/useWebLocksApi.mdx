...
description: "Coordinates asynchronous resource access using the Web Locks API."
entrypoint: rooks
status: stable
aliases: []
related: []
...
export default function SaveWithLock() {
  const [message, setMessage] = useState("Ready");
  const { acquire, isLocked, isSupported } = useWebLocksApi("save-profile");

  async function save() {
    try {
      await acquire(async () => setMessage("Saved"));
    } catch (error: unknown) {
...
function CriticalOperationComponent() {
  ...
  const { isSupported, isLocked, waitingCount, error, acquire, query } =
  ...
  const checkLockStatus = async () => {
    try {
      ...
      addLog(
        `Lock status - Held: ${status.held.length}, Pending: ${status.pending.length}`
      );
    } catch (err) {
...
function SharedResourceComponent() {
  ...
  const { isSupported, isLocked, waitingCount, error, acquire, resourceName } =
...
- **Type:** `{ periodicCheck?: boolean; checkInterval?: number }`
- **Description:** Periodic lock-snapshot configuration.
...
```typescript
{
  ...
  /** Whether this hook has an acquire request waiting or running. */
  isLocked: boolean;
  ...
  /** Abort the hook-created signal while its request is still pending. */
  release: () => void;
...
The `acquire` function accepts this inline options shape (it is not a separate public export):
...
- The hook clears its periodic timer and aborts a pending internally-signaled request on unmount
...
The hook queries once after mount and, when enabled, schedules the next query `checkInterval` milliseconds after each prior query settles. `acquire` sets `isLocked` while its request is waiting or its callback is running; it is not proof that the lock has already been granted. The lock is held until the callback settles. `release` only aborts a pending request that uses the hook-created signal—it cannot stop a running callback or release its granted lock. Cleanup clears periodic checks and aborts that pending internal request.

## Compatibility and accessibility

SSR and unsupported browsers set `isSupported: false`; `query` and `acquire` then store and throw an error. API, query, acquisition, and callback failures are likewise stored and rethrown. Do not combine `signal` with `ifAvailable`. The hook does not expose the nullable `Lock` passed by the platform, so its callback still runs when an `ifAvailable` request is delivered without a lock; do not use that mode for work that must be mutually exclusive.
