...
description: "Enables cross-tab/window communication via the BroadcastChannel API."
entrypoint: rooks
status: stable
aliases: []
related: []
...
export default function CrossTabStatus() {
  const [lastMessage, setLastMessage] = useState("No messages yet");
  const { isSupported, postMessage } = useBroadcastChannel<string>(
    "rooks-docs",
  ...
  return (
    <section>
      <p>{isSupported ? lastMessage : "BroadcastChannel is unavailable"}</p>
      <button
        type="button"
        onClick={() => postMessage("Hello from another tab")}
      >
        Broadcast message
      </button>
    </section>
...
| ------------- | ------------------------------------------------------------------------------ | ----------------------------------------------- | -------- |
| `channelName` | `string`                                                                       | The name of the broadcast channel to connect to | required |
| `options`     | `{ onMessage?: (data: T) => void; onError?: (error: Event \| Error) => void }` | Message and error callbacks                     | `{}`     |
...
| onError   | (error: Event \| Error) => void | Called for message deserialization, construction, or send errors | undefined |
...
- Other open channel objects with the same name receive the messages; the sending object does not receive its own post
- Consider using unique channel names for different features to avoid message conflicts

## Behavior and lifecycle

The effect creates one channel, listens for `message` and `messageerror`, and closes it when the name or callbacks change or the component unmounts. `postMessage` uses the structured-clone algorithm. Calling `close` closes the current channel immediately; it is not reopened until an effect dependency changes or the hook remounts. A sender does not receive its own message through the same channel object.

## Compatibility and accessibility

SSR and unsupported browsers set `isSupported: false`; send/close calls then do nothing, with send warnings in development consoles. Creation and structured-clone failures are reported to `onError` when they are `Event` or `Error` objects. Validate received data as untrusted same-origin-context input and provide an in-page status when synchronization matters.
