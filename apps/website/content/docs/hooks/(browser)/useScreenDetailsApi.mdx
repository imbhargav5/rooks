...
description: "Provides details about the user's screens using the Screen Details API."
entrypoint: rooks
status: stable
aliases: []
related: []
    ...
    <div>
      ...
      {currentScreen && (
        <div>
          ...
          <p>
            Resolution: {currentScreen.width}x{currentScreen.height}
          </p>
          <p>
            Available area: {currentScreen.availWidth}x
    ...
    <div>
      ...
      {primaryScreen && (
        <div style={{ padding: "10px", backgroundColor: "#e8f5e8" }}>
          <p>
            <strong>{primaryScreen.label}</strong>
          </p>
          <p>
            Resolution: {primaryScreen.width}x{primaryScreen.height}
          </p>
    ...
    <div>
      ...
      >
        {screens.map((screen, index) => (
          ...
          >
            <div style={{ fontWeight: "bold" }}>{screen.label}</div>
            <div>
              {screen.width}×{screen.height}
            </div>
            <div>{screen.isPrimary ? "Primary" : "Secondary"}</div>
...
| ----------------- | --------------------- | ---------------------------------------------------- |
| screens           | screen objects[]      | Array of all available screens                       |
| currentScreen     | screen object \| null | Current screen where the browser window is displayed |
| primaryScreen     | screen object \| null | Primary screen (the main display)                    |
| externalScreens   | screen objects[]      | External screens (non-primary screens)               |
...
| Option           | Type      | Default | Description                                                        |
| ---------------- | --------- | ------- | ------------------------------------------------------------------ |
| `requestOnMount` | `boolean` | `false` | Call the permission flow from an effect after mount.               |
| `autoRefresh`    | `boolean` | `true`  | Refresh after screen/current-screen or granted-permission changes. |

## Behavior and lifecycle

`requestPermission` first queries `window-management`, then calls `getScreenDetails` for both `"prompt"` and `"granted"` states. Success stores the screen collection and marks permission granted; denial/failure clears that flag and stores a message. `refresh` is a no-op until permission is marked granted. Screen-details and permission listeners are removed when their source changes or the component unmounts.

## Compatibility and accessibility

SSR and unsupported browsers start with empty lists and `isSupported: false`; asking for permission then sets an unsupported error. The API is experimental, secure-context-only, and exposes privacy-sensitive multi-screen geometry. Prefer a user-triggered request over `requestOnMount`, explain the use, provide a single-screen fallback, and avoid assuming screen labels or coordinates are stable.
