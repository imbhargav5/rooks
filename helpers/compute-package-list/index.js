...
(async () => {
    ...
    const {stdout} = await execa.command(`lerna exec --ignore rooks --ignore shared --ignore docusaurus --ignore eslint-config-rooks -- node ../../helpers/compute-package-list/get-package-info.js`);
    ...
    const pkgList = stdout.split("\n").map(package => {
        ...
        return {
            name, description, tags,hookName,docsPath
        }
    })
    const markdownTemplate = ({hookName, description, docsPath}) => `[${hookName}](${docsPath}) - ${description}`    
