...
describe("useWebLocksApi", () => {
  ...
  it("should validate resource name as string", () => {
    ...
    // Valid string resource name
    expect(() => renderHook(() => useWebLocksApi("valid-resource"))).not.toThrow();
    
    // Invalid resource names should throw
    expect(() => renderHook(() => useWebLocksApi(123 as any))).toThrow();
    expect(() => renderHook(() => useWebLocksApi(null as any))).toThrow();
    expect(() => renderHook(() => useWebLocksApi(undefined as any))).toThrow();
    expect(() => renderHook(() => useWebLocksApi({} as any))).toThrow();
  });

  it("should handle periodic checks when enabled", async () => {
    expect.hasAssertions();
    jest.useFakeTimers();
  ...
  it("should handle error state correctly", async () => {
    ...
    // Acquire should throw and set error state
    await act(async () => {
      await expect(result.current.acquire(mockCallback)).rejects.toThrow(
        "Lock acquisition failed"
      );
  ...
  it("should handle query errors and set error state", async () => {
    expect.hasAssertions();
  ...
  it("should update resource name in return object", () => {
    expect.hasAssertions();
    const { result, rerender } = renderHook(
      ({ resourceName }) => useWebLocksApi(resourceName),
      { initialProps: { resourceName: "resource-1" } }
  ...
  it("should handle custom check interval", async () => {
    expect.hasAssertions();
    jest.useFakeTimers();
