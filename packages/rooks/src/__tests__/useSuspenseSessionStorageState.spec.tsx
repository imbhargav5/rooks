...
function TestComponent<T>({
    ...
    const [value, { getItem, setItem, deleteItem }] = useSuspenseSessionStorageState(
        storageKey,
        initializer
    );
...
function ObjectTestComponent({ storageKey }: { storageKey: string }) {
    const [user, { setItem, deleteItem }] = useSuspenseSessionStorageState(
        storageKey,
        (value) => {
            if (value) {
                try {
...
describe("useSuspenseSessionStorageState", () => {
    beforeEach(() => {
    ...
    it("should be defined", () => {
        ...
        expect(useSuspenseSessionStorageState).toBeDefined();
    });
    ...
    it("should initialize with default value when sessionStorage is empty", async () => {
        expect.hasAssertions();
    ...
    it("should initialize with existing value from sessionStorage", async () => {
        expect.hasAssertions();

        // Pre-populate sessionStorage
        sessionStorage.setItem("test-existing", JSON.stringify("existing-value"));
    ...
    it("should work with number types", async () => {
        ...
        const storedValue = sessionStorage.getItem("test-number");
    ...
    it("should work with object types", async () => {
        ...
        const storedValue = sessionStorage.getItem("test-object");
    ...
    it("should reset to initial value when deleteItem is called", async () => {
        ...
        expect(screen.getByTestId("count").textContent).toBe("0");
        expect(sessionStorage.getItem("test-delete")).toBeNull();
    });
    ...
    it("should handle sessionStorage errors gracefully", async () => {
        ...
        // Should still work even if sessionStorage throws
        await act(async () => {
            fireEvent.click(screen.getByTestId("increment"));
        });
    ...
    it("should handle initializer errors and show error boundary", async () => {
        ...
        const BadComponent = () => {
            useSuspenseSessionStorageState("test-bad", () => {
    ...
    it("should handle malformed JSON in sessionStorage", async () => {
        expect.hasAssertions();

        // Pre-populate sessionStorage with malformed JSON
        sessionStorage.setItem("test-malformed", "{ invalid json");
    ...
    it("should handle server-side rendering scenario", async () => {
        ...
        // Mock sessionStorage as undefined (SSR scenario)
        ...
        // Should work with default value even without sessionStorage
