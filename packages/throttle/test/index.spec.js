...
describe('useThrottle hook', () => {
  ...
  beforeEach(() => {
    App = function() {
      ...
      const addNumberThrottledWithParam = useThrottle(() => addNumberWithParam(5), 1000);
      return (
        <div>
          <span data-testid="throttle-value">{number}</span>
          <span data-testid="throttle-value-with-param">{argumentNumber}</span>
          <button data-testid="throttle-button" onClick={addNumberThrottled}>Add number throttled</button>
          <button data-testid="throttle-button-with-param" onClick={addNumberThrottledWithParam}>Add number throttled</button>
        </div>
      );
    }
  });
